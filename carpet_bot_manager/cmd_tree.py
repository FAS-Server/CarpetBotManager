...
def register_command(server: PluginServerInterface, manager: BotManager):
    existed_bot_node = Text('bot').requires(
        lambda src, ctx: manager.check_list(src, ctx['bot']),
        lambda src: Message.Command.unknown_bot
    ).suggests(manager.bots_in_list)


    player_node = Literal(constants.Prefix2).runs(lambda src: src.reply(Message.Help.general)) \
        .on_error(
        ...
        Text('bot').suggests(manager.bots_in_list)
            .runs(lambda src: src.reply(Message.Command.unknown_argument)).then(
    ...
    node = Literal(constants.Prefix).runs(lambda src: src.reply(Message.Help.general())) \
        .on_error(
        UnknownArgument, lambda src: src.reply(Message.Command.unknown_argument), handled=True
    ).then(
        ...
        Literal('spawn').runs(lambda src: src.reply(Message.Command.bot_name_required))
            .then(
        ...
        Literal('action').runs(lambda src: src.reply(Message.Help.action()))
            .then(
            existed_bot_node.runs(lambda src, ctx: manager.list_action(src, ctx['bot']))
                .then(
                Literal('clear')
                    .runs(lambda src, ctx: manager.clear_action(src, ctx['bot']))
            ).then(
                Literal('exec')
                    .runs(lambda src, ctx: manager.exec_action(src, ctx['bot']))
            )
        )
    ).then(
        Literal('del').runs(lambda src: src.reply(Message.Command.bot_name_required))
            .then(
            existed_bot_node.runs(lambda src, ctx: manager.del_bot(src, ctx['bot']))
        )
    ).then(
        Literal('info').runs(lambda src: src.reply(Message.Command.bot_name_required))
            .then(
            existed_bot_node.runs(lambda src, ctx: manager.info_bots(src, ctx['bot']))
        )
    ).then(
        Literal('desc').runs(lambda src: src.reply(Message.Command.bot_name_required))
            .then(
            existed_bot_node.runs(lambda src, ctx: manager.info_bots(src, ctx['bot']))
                .then(
    ...
    server.register_help_message(constants.Prefix, 'summery_help')
