...
def register_command(server: PluginServerInterface, manager: BotManager):
    ...
    builder.command("!!bot actions", lambda src: src.reply(action_help))
    builder.command("!!bot actions <ebot>", lambda src, ctx: manager.list_action(src, ctx['ebot']))
    builder.command("!!bot actions <ebot> clear", lambda src, ctx: manager.clear_action(src, ctx['ebot']))
    builder.command("!!bot actions <ebot> exec", lambda src, ctx: manager.exec_action(src, ctx['ebot']))
